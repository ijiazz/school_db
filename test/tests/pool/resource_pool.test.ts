...
test("最大连接数量 默认为 3", async function ({ resourceManage }) {
  ...
  const connList = await Promise.all([pool.connect(), pool.connect(), pool.connect()]);
...
test("close() 后尝试再连接应抛出异常", async function ({ pool }) {
  const conn = pool.connect().catch(() => {});
  const closePromise = pool.close();
  expect(pool.closed).toBe(true);
  await expect(pool.connect()).rejects.toThrowError();
...
test("close(true) 会立即断开所有连接", async function ({ resourceManage }) {
  const pool = new ResourcePool(resourceManage, { maxCount: 2 });
  const conn1 = pool.connect();
  const conn2 = pool.connect();
  const conn3 = pool.connect();
...
test("串行获取50次连接", async function ({ resourceManage, pool }) {
  ...
  expect(pool.waitingCount).toBe(0);
  expect(pool.idleCount).toBe(1);
  expect(resourceManage.connect).toBeCalledTimes(1);
});
test("并行获取50次连接", async function ({ resourceManage, pool }) {
  ...
  expect(resourceManage.connect).toBeCalledTimes(ResourcePool.defaultMaxCount);
});
...
describe("连接中途断开", function () {
  test("空闲时断开", async function ({ pool, resourceManage }) {
    const conn = await pool.connect();
    pool.release(conn);
    pool.disposeConn(conn);
    expect(resourceManage.disconnect).not.toBeCalled();
    expect(pool.totalCount).toBe(0);
    expect(pool.idleCount).toBe(0);
  });
