...
CREATE TRIGGER sync_pla_comment_resource_ref_count -- 评论头像快照
AFTER INSERT
OR DELETE
OR
UPDATE ON pla_comment FOR EACH ROW EXECUTE FUNCTION resource_ref_sync ();

/* CREATE TRIGGER sync_comment_image_ref_count -- 评论图片引用
AFTER INSERT
OR DELETE
OR
UPDATE ON comment_image FOR EACH ROW EXECUTE FUNCTION resource_ref_sync (); */
